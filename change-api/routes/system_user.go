...
func SystemUser(rg *gin.RouterGroup) gin.IRoutes {
	rs := rg.Group("/user")
	{
		...
		rs.PUT("/info/:username", v1.UpdateUserInfoByUsernameHandler)          // 更新指定用户信息
		...
		rs.PUT("/change/status")                                               // 禁用，启用，锁定，解锁当前用户，注意清理 Redis
		rs.PUT("/change/status/:username")                                     // 禁用，启用，锁定，解锁指定用户
